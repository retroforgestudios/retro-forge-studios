...
- [x] **Background treatment** — done, revised. Sections now alternate between `#202020` (base, dark) and `#f2f2f2` (`bg-band` utility, [`src/styles/global.css`](src/styles/global.css)) — updated from an earlier all-dark version after feedback that the site felt too dark overall. This is a deviation from the brief's "primary background: charcoal everywhere" spec; flag if you'd rather revert to all-dark.

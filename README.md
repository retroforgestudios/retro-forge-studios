...
- [x] **Trust-bar tool logos** — done. `TRUST_TOOLS` now lists HubSpot, Claude, YouTube, WordPress, Gemini, TikTok, rendered with inline-SVG marks via [`src/components/ui/ToolIcon.astro`](src/components/ui/ToolIcon.astro). Confirm this is the final list, and note these are hand-built approximations of each brand mark, not official logo files — swap in real assets if precise brand compliance matters.
